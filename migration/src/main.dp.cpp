...
int main(int argc, char* argv[]) {
    ...
    tmr->time = 0.0f;
/* DPCT_ORIG     tmr->iter = scn.sourceRange.x;*/
    tmr->iter = scn.sourceRange.x();
/* DPCT_ORIG     while(tmr->iter <= scn.renderRange.y) {*/
    while (tmr->iter <= scn.renderRange.y()) {
/* DPCT_ORIG         if (tmr->iter <= scn.sourceRange.y) {*/
        if (tmr->iter <= scn.sourceRange.y()) {
            solver->addSource();
        }
        solver->step();
/* DPCT_ORIG         if (tmr->iter >= scn.renderRange.x && tmr->iter <=
 * scn.renderRange.y) {*/
        if (tmr->iter >= scn.renderRange.x() &&
            tmr->iter <= scn.renderRange.y()) {
